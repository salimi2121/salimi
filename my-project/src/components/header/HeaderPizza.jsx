import flogo from "../../assets/logo-white2.png";
import Logo from "./Logo";
import React, { useEffect, useState } from 'react';
import Cartbtn from "./Cartbtn";
import HamburgerMenu from "../header/Hambergermenu";
import NavbtnPizza from "./NavbtnPizza";
import NavmenuPizza from "./NavmenuPizza";
    ...
    return (
        ...
        <div className={`fixed w-full max-w-full flex xl:px-30
             ...
             max-[530px]:px-7 max-[440px]:px-4
              justify-between items-center z-40 max-[1285px]:px-4
            ${isFixed ? 'top-0 bg-black shadow-lg max-[1280px]:py-4 transition-all duration-300 ease-in-out' : 'top-4 bg-transparent'}   
            ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-[-220px] transition-all duration-300 ease-in'}`}>
            <Logo imageUrl={flogo} />
            <div className="flex xl:gap-14 text-white max-[530px]:gap-10 max-[440px]:gap-0 items-center justify-center max-[530px]:w-30">
                <NavmenuPizza subMenus={subMenus} />
                <div className="flex w-fit">
                    <div className="xl:mr-4 hidden xl:inline-block">
                        <NavbtnPizza title="Book a table" />
                    </div>
                    <Cartbtn />
